...
### 3.2 Phase plan 작성
`.workflow/<작업번호>/plans/` 디렉토리 생성. 3개 phase plan 파일 작성:
...
선택된 phase의 plan 파일 (`.workflow/<작업번호>/plans/<phase>-phase.md`)을 따라 코드 작성:
...
- 완료 신호는 code-phases.json의 세 phase가 모두 done인 것과 git 커밋으로 반영된다 (work.json에는 기록하지 않음).
- 종료 안내:
  ```
