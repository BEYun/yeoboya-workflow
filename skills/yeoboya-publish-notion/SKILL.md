...
- **upsert 규칙**: `work.json.links[<key>]`(다중 페이지는 `links[<key>][<title>]`)가 있으면 update, 없으면 create. 이 판단은 **동기화된 links**를 전제로 한다 — 호출자(작업목록 skill)는 진입 시 `mode="sync-links"`로 links를 먼저 최신화하므로, 다른 작업자가 만든 기존 페이지도 create가 아니라 update된다(중복 방지).
