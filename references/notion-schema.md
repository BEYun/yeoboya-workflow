...
**QA 시나리오(write-qa)만 예외**: 마크다운 자식 페이지가 아니라 작업 row **자식 데이터베이스**로 생성된다(제목 "QA 시나리오"). 행=테스트 케이스, 컬럼=`케이스 ID`(title)·`페르소나`·`카테고리`·`시나리오`·`관련 API / Socket`·`테스트 결과`(`페르소나`는 도메인 명세서 집합 기반 동적 select, 그 외 select 옵션은 write-qa SKILL §3). 행 본문에 `사전 조건`/`테스트 단계`/`기대 결과`. `테스트 결과`는 신규 행 `미테스트`이고 QA 수행 시 사람이 채운다.
